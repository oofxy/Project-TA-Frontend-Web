...
import { postPendidikan, patchPendidikan } from "@/data/data-master/pendidikan";
...
export default function CustomDialog({
  ...
  const onSubmit = async (data: DataMasterSchema) => {
    ...
    try {
      if (mode === "edit" && id) {
        await patchPendidikan(id, { name: data.name });
      } else {
        await postPendidikan({
          name: data.name,
          id: "",
        });
      }
      ...
      toast.success(
        `Pendidikan berhasil ${mode === "add" ? "ditambahkan" : "diperbarui"}`
      );
      reset();
      setOpen(false);
      router.refresh();
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mb-5">
            {mode === "add" ? "Tambah Pendidikan Baru" : "Edit Pendidikan"}
          </DialogTitle>
        </DialogHeader>
