...
import { postKelurahan, patchKelurahan } from "@/data/data-master/kelurahan";
...
export default function CustomDialog({
  ...
  const onSubmit = async (data: DataMasterSchema) => {
    ...
    try {
      if (mode === "edit" && id) {
        await patchKelurahan(id, { name: data.name });
      } else {
        await postKelurahan({
          name: data.name,
          id: "",
      ...
      toast.success(
        `Kelurahan berhasil ${mode === "add" ? "ditambahkan" : "diperbarui"}`
      );
      reset();
      setOpen(false);
      router.refresh();
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mb-5">
            {mode === "add" ? "Tambah Kelurahan Baru" : "Edit Kelurahan"}
          </DialogTitle>
        </DialogHeader>
