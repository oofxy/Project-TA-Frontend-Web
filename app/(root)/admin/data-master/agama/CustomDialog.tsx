...
import { postAgama, patchAgama } from "@/data/data-master/agama";
...
export default function CustomDialog({
  ...
  const onSubmit = async (data: DataMasterSchema) => {
    ...
    try {
      if (mode === "edit" && id) {
        await patchAgama(id, { name: data.name });
      } else {
        await postAgama({
          name: data.name,
          id: "",
      ...
      toast.success(
        `Agama berhasil ${mode === "add" ? "ditambahkan" : "diperbarui"}`
      );
      reset();
      setOpen(false);
      router.refresh();
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mb-5">
            {mode === "add" ? "Tambah Agama Baru" : "Edit Agama"}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-6">
          <div className="space-y-4">
            <div className="flex flex-col gap-3">
              <Label htmlFor="name">Nama Agama</Label>
              <Input
                id="name"
