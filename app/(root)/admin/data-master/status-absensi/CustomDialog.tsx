...
export default function CustomDialog({
  ...
  const [formData, setFormData] = useState({
    statusAbsensi: statusAbsensi?.statusAbsensi || "",
  });
          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">
                {mode === "add" ? "Tambah" : "Edit"} Status Absensi
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                ...
                <input
                  type="text"
                  value={formData.statusAbsensi}
                  onChange={(e) =>
                    setFormData({ ...formData, statusAbsensi: e.target.value })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                  required
                  placeholder="Masukkan nama status absensi"
                  title="Nama Status Absensi"
                />
              </div>
