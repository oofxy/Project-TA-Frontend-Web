...
export default function JenisKelaminPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch("/api/data/data-master/jenis-kelamin", {
        method: "GET",
      });
      const json = await res.json();
      setData(json);
    } catch (err) {
      toast.error("Gagal fetch data jenis kelamin");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmit = async (formData: DataMasterSchema) => {
    ...
    const url = id
      ? `/api/data/data-master/jenis-kelamin/${id}`
      : "/api/data/data-master/jenis-kelamin";
    ...
    try {
      ...
      toast.success(
        `jenis kelamin berhasil ${id ? "diperbarui" : "ditambahkan"}`
      );
      reset();
      await fetchData();
      setOpen(false);
  ...
  return (
    <div className="h-full">
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="mb-5">
                {id ? "Edit jenis kelamin" : "Tambah jenis kelamin Baru"}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div className="flex flex-col gap-3">
                <Label htmlFor="name">Nama jenis kelamin</Label>
                <Input
                  id="name"
      ...
      <div className="h-full">
        <TableData
          columns={jenisKelamin({ fetchData, handleEdit })}
          data={data}
        />
      </div>
    </div>
