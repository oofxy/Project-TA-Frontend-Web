...
import { postJenisKelamin, patchJenisKelamin } from "@/data/data-master/jenis-kelamin";
...
export default function CustomDialog({
  ...
  const onSubmit = async (data: DataMasterSchema) => {
    ...
    try {
      if (mode === "edit" && id) {
        await patchJenisKelamin(id, { name: data.name });
      } else {
        await postJenisKelamin({
          name: data.name,
          id: "",
        });
      }
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mb-5">
            {mode === "add" ? "Tambah JenisKelamin Baru" : "Edit JenisKelamin"}
          </DialogTitle>
        </DialogHeader>
