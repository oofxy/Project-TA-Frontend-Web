...
import { postGolongan, patchGolongan } from "@/data/data-master/golongan";
...
export default function CustomDialog({
  ...
  const onSubmit = async (data: DataMasterSchema) => {
    ...
    try {
      if (mode === "edit" && id) {
        await patchGolongan(id, { name: data.name });
      } else {
        await postGolongan({
          name: data.name,
          id: "",
      ...
      toast.success(
        `Golongan berhasil ${mode === "add" ? "ditambahkan" : "diperbarui"}`
      );
      reset();
      setOpen(false);
      router.refresh();
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mb-5">
            {mode === "add" ? "Tambah Golongan Baru" : "Edit Golongan"}
          </DialogTitle>
        </DialogHeader>
