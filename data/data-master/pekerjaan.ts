...
export async function getPekerjaan(): Promise<DataMaster[]> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}pekerjaan`, {
    headers: {
...
export async function postPekerjaan(data: DataMaster): Promise<DataMaster> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}pekerjaan`, {
    method: "POST",
    headers: {
      Accept: "application/json",
...
export async function patchPekerjaan(
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}pekerjaan/${id}`, {
    method: "PATCH",
    headers: {
      Accept: "application/json",
      Authorization: `Bearer ${process.env.NEXT_PUBLIC_API_TOKEN}`,
