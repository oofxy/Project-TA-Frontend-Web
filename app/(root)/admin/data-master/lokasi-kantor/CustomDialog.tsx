...
import { postLokasiKantor, patchLokasiKantor } from "@/data/data-master/lokasi-kantor";
...
export default function CustomDialog({
  ...
  const onSubmit = async (data: DataMasterSchema) => {
    ...
    try {
      if (mode === "edit" && id) {
        await patchLokasiKantor(id, { name: data.name });
      } else {
        await postLokasiKantor({
          name: data.name,
          id: "",
        });
      }
      ...
      toast.success(
        `Lokasi Kantor berhasil ${mode === "add" ? "ditambahkan" : "diperbarui"}`
      );
      reset();
      setOpen(false);
      router.refresh();
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mb-5">
            {mode === "add" ? "Tambah Lokasi Kantor Baru" : "Edit Lokasi Kantor"}
          </DialogTitle>
        </DialogHeader>
