...
import { postKabupaten, patchKabupaten } from "@/data/data-master/kabupaten";
...
export default function CustomDialog({
  ...
  const onSubmit = async (data: DataMasterSchema) => {
    ...
    try {
      if (mode === "edit" && id) {
        await patchKabupaten(id, { name: data.name });
      } else {
        await postKabupaten({
          name: data.name,
          id: "",
      ...
      toast.success(
        `Kabupaten berhasil ${mode === "add" ? "ditambahkan" : "diperbarui"}`
      );
      reset();
      setOpen(false);
      router.refresh();
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mb-5">
            {mode === "add" ? "Tambah Kabupaten Baru" : "Edit Kabupaten"}
          </DialogTitle>
        </DialogHeader>
