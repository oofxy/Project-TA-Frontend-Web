...
export default function GolonganPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch("/api/data/data-master/golongan", {
        method: "GET",
      });
      const json = await res.json();
      setData(json);
    } catch (err) {
      toast.error("Gagal fetch data golongan");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmit = async (formData: DataMasterSchema) => {
    ...
    const url = id
      ? `/api/data/data-master/golongan/${id}`
      : "/api/data/data-master/golongan";
    ...
    try {
      ...
      toast.success(`golongan berhasil ${id ? "diperbarui" : "ditambahkan"}`);
      reset();
      await fetchData();
      setOpen(false);
      setid(null);
  ...
  return (
    <div className="h-full">
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="mb-5">
                {id ? "Edit golongan" : "Tambah golongan Baru"}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div className="flex flex-col gap-3">
                <Label htmlFor="name">Nama golongan</Label>
                <Input
                  id="name"
      ...
      <div className="h-full">
        <TableData columns={golongan({ fetchData, handleEdit })} data={data} />
      </div>
    </div>
  );
}
