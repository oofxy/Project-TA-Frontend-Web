...
import { postProvinsi, patchProvinsi } from "@/data/data-master/provinsi";
...
export default function CustomDialog({
  ...
  const onSubmit = async (data: DataMasterSchema) => {
    ...
    try {
      if (mode === "edit" && id) {
        await patchProvinsi(id, { name: data.name });
      } else {
        await postProvinsi({
          name: data.name,
          id: "",
      ...
      toast.success(
        `Provinsi berhasil ${mode === "add" ? "ditambahkan" : "diperbarui"}`
      );
      reset();
      setOpen(false);
      router.refresh();
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mb-5">
            {mode === "add" ? "Tambah Provinsi Baru" : "Edit Provinsi"}
          </DialogTitle>
        </DialogHeader>
