...
export default function StatusAbsensiPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch("/api/data/data-master/status-absensi", {
        method: "GET",
      });
      const json = await res.json();
      setData(json);
    } catch (err) {
      toast.error("Gagal fetch data status absensi");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmit = async (formData: DataMasterSchema) => {
    ...
    const url = id
      ? `/api/data/data-master/status-absensi/${id}`
      : "/api/data/data-master/status-absensi";
    ...
    try {
      ...
      toast.success(
        `status absensi berhasil ${id ? "diperbarui" : "ditambahkan"}`
      );
      reset();
      await fetchData();
      setOpen(false);
  ...
  return (
    <div className="h-full">
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="mb-5">
                {id ? "Edit status absensi" : "Tambah status absensi Baru"}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div className="flex flex-col gap-3">
                <Label htmlFor="name">Nama status absensi</Label>
                <Input
                  id="name"
                  placeholder="Masukkan nama status absensi"
                  {...register("name")}
                />
                {errors.name && (
      ...
      <div className="h-full">
        <TableData
          columns={statusAbsensi({ fetchData, handleEdit })}
          data={data}
        />
      </div>
    </div>
