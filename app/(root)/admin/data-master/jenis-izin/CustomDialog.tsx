...
import { postJenisIzin, patchJenisIzin } from "@/data/data-master/jenis-izin";
...
export default function CustomDialog({
  ...
  const onSubmit = async (data: DataMasterSchema) => {
    ...
    try {
      if (mode === "edit" && id) {
        await patchJenisIzin(id, { name: data.name });
      } else {
        await postJenisIzin({
          name: data.name,
          id: "",
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mb-5">
            {mode === "add" ? "Tambah JenisIzin Baru" : "Edit JenisIzin"}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-6">
          <div className="space-y-4">
            <div className="flex flex-col gap-3">
              <Label htmlFor="name">Nama Jenis Izin</Label>
              <Input
                id="name"
