...
import { postDivisi, patchDivisi } from "@/data/data-master/divisi";
...
export default function CustomDialog({
  ...
  const onSubmit = async (data: DataMasterSchema) => {
    ...
    try {
      if (mode === "edit" && id) {
        await patchDivisi(id, { name: data.name });
      } else {
        await postDivisi({
          name: data.name,
          id: "",
      ...
      toast.success(
        `Divisi berhasil ${mode === "add" ? "ditambahkan" : "diperbarui"}`
      );
      reset();
      setOpen(false);
      router.refresh();
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mb-5">
            {mode === "add" ? "Tambah Divisi Baru" : "Edit Divisi"}
          </DialogTitle>
        </DialogHeader>
